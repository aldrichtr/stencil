<#
.SYNOPSIS
  Present a menu of choices to select.
.DESCRIPTION
  The `choice` operation presents a list of choices (either single or multi choice).  When the option(s) are
  selected the choice is saved in the `Output` field for use by other operations within the job.
.PARAMETER MultiChoice The choice menu accepts multiple selections

.PARAMETER Default Set an option (by index) as the default choice in the menu
#>
