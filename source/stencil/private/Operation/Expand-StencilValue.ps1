...
function Expand-StencilValue {
  ...
  begin {
    ...
    $keyNotFound = $false
    # A token is identified by a dollar sign and sorrounding braces, like '${<token>}
    $tokenPattern = '\$\{(?<var>.+?)\}'
  ...
  process {
    <#------------------------------------------------------------------------------------------------
      ...
      - First, the input ($Value) could be multi-line text, so we need to loop over each line.
      - In the current line, look for all the matches to the pattern '${}' and loop over each match
      - for this match, if the match was a multi-level value (i.e. env.foo) loop over each level
    ------------------------------------------------------------------------------------------------#>
    if (-not ($PSBoundParameters.ContainsKey('Data'))) {
      $possibleData = $PSCmdlet.GetVariableValue('Job')
      if ($null -eq $possibleData) {
        throw 'No data given for expanding stencil values'
        $Data = $possibleData
        $possibleData = null
      } else {
        Write-Debug '  No Job data given, using global Jobs Table'
        $Data = $PSCmdlet.GetVariableValue('Jobs')
      }
    }
